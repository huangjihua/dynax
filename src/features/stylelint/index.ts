...
export default async function (targetDir: string, template: FrameworkType, features: string[], isSass: boolean) {
  ...
  await createOrOverwriteFile(`${targetDir}/.stylelintrc.js`, `module.exports = ${JSON.stringify(isNative ? nativeLint : stylelint, null, 2).replace(/"(?!.*\/)(?!.*-)([^"]+)":/g, '$1:')}`)
  await createOrOverwriteFile(`${targetDir}/.stylelintignore`, stylelintignore)
